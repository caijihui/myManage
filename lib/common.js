    ...
    for (i in jsonObj) {//json 对象
        if(jsonObj[i] === null)
        {
          ...
          if (typeof jsonObj[i] === 'string') {
            retVal += tGlue +i+" = '"+ addslashes(jsonObj[i])+"'";
          } else {
            retVal += tGlue +i+" = '"+ jsonObj[i]+"'";
          }
        }
...
exports.toWhereFieldsByOr = function (jsonObj)
{
  var retVal ='';
  var i = ''
  var tGlue = ''
...
exports.toInsertFields = function (jsonObj)
{
    ...
    for (i in jsonObj) {//json 对象
        ...
        if(jsonObj[i] === null)
        {
          ...
          if (typeof jsonObj[i] === 'string') {
            values += tGlue+"'"+addslashes(jsonObj[i])+"'";
          } else {
            values += tGlue+"'"+jsonObj[i]+"'";
          }
        }
...
let yearCode = ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J']
exports.createOrderNo = function ( moment, orderSN = 'C') {
    var todayMoment = moment||Moment()
    orderSN += yearCode[todayMoment.format('YYYY')%10]
    orderSN += todayMoment.format('YYYYMMDDHHmmss')+todayMoment.format('SSS')+Math.floor(Math.random() * 90000 +10000);
    return orderSN
}
